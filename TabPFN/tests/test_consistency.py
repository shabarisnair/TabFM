...
The float64 predictions can still vary slightly between platforms, thus we maintain
separate reference predictions for all the platforms in `ENABLED_PLATFORMS`. The
tests are skipped on other platforms.
TODO: it may be that float64 predictions are consistent even across hardware. Check
this when regenerating references on other platforms — if so, the per-platform
reference sets can be collapsed into a single shared one.
